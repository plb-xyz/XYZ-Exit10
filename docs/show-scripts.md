...
const showScripts = {
  ...
  show_1: {
    ...
    onStart: [
      {
        label: "Start A1 video transition",
        target: "a1.video",
        action: "video.play",
        params: { timelineKey: "transition_a1" }
      },
      {
        label: "Start A2 video transition",
        target: "a2.video",
        action: "video.play",
        params: { timelineKey: "transition_a2" }
      },
      {
        label: "Start A3 video transition",
        target: "a3.video",
        action: "video.play",
        params: { timelineKey: "transition_a3" }
      },
      {
    ...
    // ── Phase 2: fires when Transition Gate releases ────────
    // Gate fires when all 3 atriums confirm opaque via TCP,
    // OR after 3s timeout from first receipt (one-shot).
    // Show 1 runs as a single unified timeline for all xyz
    // video surfaces, show audio, and MA timecode.
    onCue: {
      all_opaque: [
        {
          label: "Start Show 1 unified timeline",
    ...
    // ── Phase 3: fires when Watchout sends "show_end" ───────
    onEnd: [
      {
